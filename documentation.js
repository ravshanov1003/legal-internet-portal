...
async function createJSONFile() {
    try {
        ...
        const response = await require('./Legal-internet-portal.postman_collection.json')
        const json = await response.json()
        await fs.writeFile("./Legal-internet-portal.postman_collection.json", JSON.stringify(json));
...
(async function createYAMLFile() {
    ...
    autoToSwagger.loadFile("./Legal-internet-portal.postman_collection.json", function(err) {
        if (err) {
        ...
        autoToSwagger.convert('yaml')
            .then(function(convertedData) {
                ...
                fs.writeFile("./Legal-internet-portal.postman_collection.json", convertedData, function(err) {
                    if (err) {
...
const ymlfile = YAML.load('./Legal-internet-portal.postman_collection.json');
